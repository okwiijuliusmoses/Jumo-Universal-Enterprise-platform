// kernel/registry/factory.ts
import { 
  EnterprisePlatformDefinition, 
  PortalDefinition, 
  DirectorateDefinition, 
...
export class EnterprisePlatformFactory {
  static createPlatform(
    ...
    const portals: PortalDefinition[] = Array.from({ length: portalCount }, (_, i) => ({
      id: `portal-${i}`,
      name: `Portal ${i + 1}`,
      owner: `Owner ${i + 1}`,
    ...
    const departments: DepartmentDefinition[] = Array.from({ length: departmentCount }, (_, i) => ({
      id: `dept-${i}`,
      name: `Department ${i + 1}`,
    ...
    const modules: ModuleDefinition[] = Array.from({ length: moduleCount }, (_, i) => ({
      id: `mod-${i}`,
      name: `Module ${i + 1}`,
    ...
    const forms: FormDefinition[] = Array.from({ length: formCount }, (_, i) => ({
      id: `form-${i}`,
      name: `Form ${i + 1}`,
