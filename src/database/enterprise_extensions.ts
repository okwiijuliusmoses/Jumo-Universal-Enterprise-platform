...
export function calculateDynamicFee(amount: number = 0, config: BillingConfig): { platformFee: number; netAmount: number; details: string } {
  const safeAmount = Number(amount) || 0;
  let fee = 0;
  let details = "";
  if (config.feeType === "percentage") {
    fee = parseFloat((safeAmount * ((config.feePercentage || 0) / 100)).toFixed(2));
    details = `${config.feePercentage}% Transaction Commission applied.`;
  } else if (config.feeType === "fixed") {
    fee = config.fixedFeeAmount || 0;
    ...
    fee = parseFloat((safeAmount * 0.002).toFixed(2));
    ...
    const pctFee = parseFloat((safeAmount * ((config.feePercentage || 0) / 100)).toFixed(2));
    fee = pctFee + (config.fixedFeeAmount || 0);
    details = `Contract combination fee applied: ${config.feePercentage}% ($${pctFee}) + flat $${config.fixedFeeAmount} platform transaction fee.`;
  }
  
  if (fee > safeAmount) fee = safeAmount; // guard
  return {
    platformFee: fee,
    netAmount: parseFloat((safeAmount - fee).toFixed(2)),
    details
  };
}
