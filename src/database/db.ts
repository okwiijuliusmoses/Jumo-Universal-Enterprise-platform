  ...
  private async bootstrapPostgresTables() {
    ...
    try {
      ...
      await this.pool.query(`
        ...
        CREATE TABLE IF NOT EXISTS ueos_journal_entries (
          id VARCHAR(100) NOT NULL,
          tenant_id VARCHAR(100) NOT NULL,
          workspace_id VARCHAR(100) NOT NULL,
          posting_date DATE,
        ...
        CREATE TABLE IF NOT EXISTS ueos_journal_lines (
          id VARCHAR(100) NOT NULL,
          tenant_id VARCHAR(100) NOT NULL,
    ...
    if (schema) {
      ...
      if (pkField) {
        ...
        if (pkValue !== undefined && pkValue !== null) {
          const recTenant = (record as any).tenant_id ?? (record as any).tenantId;
          const exists = this.data[tableName].some(item => {
            const samePk = (item as any)[pkField.name] === pkValue;
            if (!samePk) return false;
            const itemTenant = (item as any).tenant_id ?? (item as any).tenantId;
            // If either record specifies a tenant, scope uniqueness to that tenant
            if (recTenant !== undefined || itemTenant !== undefined) {
              return recTenant === itemTenant;
            }
            return true;
          });
          if (exists) {
