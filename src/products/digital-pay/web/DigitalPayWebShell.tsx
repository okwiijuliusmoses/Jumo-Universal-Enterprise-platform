...
export const DigitalPayWebShell: React.FC<{ onNavigate?: (route: string) => void }> = ({ onNavigate }) => {
  ...
  const [activePortalId, setActivePortalId] = useState<string>('DP-PORTAL-MERCHANT-0001');
  const [activeTab, setActiveTab] = useState<string>('MOD_DP_DASHBOARD');
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-lg max-w-md w-full">
        ...
        <div className="space-y-4">
          <button
            type="button"
            onClick={() => {
              ownerVerificationService.establishOwnerVerificationSession('JUMO-FINPAY', 'JUMO Digital Pay Verification Access');
              setActivePortalId('DP-PORTAL-MERCHANT-0001');
              setActiveTab('MOD_DP_DASHBOARD');
              setAppState('APP');
            }}
            className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl shadow-sm transition flex items-center justify-center gap-2 cursor-pointer"
          >
                ...
                <div className="space-y-0.5">
                  {group.items.map(item => {
                    ...
                    return (
                      ...
                      >
                        <Icon className={`w-4 h-4 ${isSelected || (false && activePortalId === item.id) ? 'text-indigo-600' : 'text-slate-400'}`} />
