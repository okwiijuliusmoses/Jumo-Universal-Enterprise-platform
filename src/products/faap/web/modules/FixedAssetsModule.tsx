    ...
    <div className="space-y-6 animate-in fade-in duration-300 pb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-3xl font-black text-slate-900 mt-1 font-mono">{totalCost.toLocaleString()} UGX</p>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-3xl font-black text-rose-600 mt-1 font-mono">{totalDepr.toLocaleString()} UGX</p>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-3xl font-black text-emerald-700 mt-1 font-mono">{totalNBV.toLocaleString()} UGX</p>
          ...
          <tbody className="divide-y divide-slate-100">
            {assets.map((ast) => (
              <tr key={ast.code} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4 text-xs font-mono text-slate-600">{ast.acquisitionDate}</td>
                <td className="px-6 py-4 text-right font-mono font-bold text-slate-900">{ast.cost.toLocaleString()}</td>
                <td className="px-6 py-4 text-right font-mono text-rose-600 font-bold">{ast.accumulatedDepreciation.toLocaleString()}</td>
                <td className="px-6 py-4 text-right font-mono font-black text-emerald-700">{ast.netBookValue.toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
