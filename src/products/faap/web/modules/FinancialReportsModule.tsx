                ...
                {reportData?.accountAudits?.map((acc: any) => (
                  <tr key={acc.accountCode}>
                    ...
                    <td className="py-3 text-right font-mono font-medium">
                      {acc.calculatedBalance > 0 ? acc.calculatedBalance.toLocaleString() : '-'}
                    </td>
                    <td className="py-3 text-right font-mono font-medium">
                      {acc.calculatedBalance < 0 ? Math.abs(acc.calculatedBalance).toLocaleString() : '-'}
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="border-t-2 border-slate-900">
                <tr>
                  ...
                  <td className="py-4 text-right font-black font-mono text-indigo-600">
                    {reportData?.accountAudits?.reduce((sum: number, acc: any) => sum + (acc.calculatedBalance > 0 ? acc.calculatedBalance : 0), 0).toLocaleString()}
                  </td>
                  <td className="py-4 text-right font-black font-mono text-indigo-600">
                    {Math.abs(reportData?.accountAudits?.reduce((sum: number, acc: any) => sum + (acc.calculatedBalance < 0 ? acc.calculatedBalance : 0), 0)).toLocaleString()}
                  </td>
                </tr>
              </tfoot>
            </table>
                ...
                <tbody className="divide-y divide-slate-50">
                  {reportData?.accountAudits?.filter((a: any) => a.accountCode.startsWith('4')).map((acc: any) => (
                    <tr key={acc.accountCode}>
                      ...
                      <td className="py-2 text-right font-mono font-medium">{Math.abs(acc.calculatedBalance).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="w-full text-sm">
                <tbody className="divide-y divide-slate-50">
                  {reportData?.accountAudits?.filter((a: any) => a.accountCode.startsWith('5')).map((acc: any) => (
                    <tr key={acc.accountCode}>
                      <td className="py-2 pl-4 text-slate-700">{acc.accountName}</td>
                      <td className="py-2 text-right font-mono font-medium">{acc.calculatedBalance.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between font-bold pt-2 border-t border-slate-200">
                    ...
                    <span className="font-mono">{reportData?.totalAssets?.toLocaleString()}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between font-bold pt-2 border-t border-slate-200">
                    ...
                    <span className="font-mono">{reportData?.totalLiabilities?.toLocaleString()}</span>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between font-bold pt-2 border-t border-slate-200">
                    ...
                    <span className="font-mono">{reportData?.totalEquity?.toLocaleString()}</span>
                  </div>
                </div>
                
                <div className="flex justify-between font-black text-lg pt-4 border-t-2 border-slate-900">
                  <span>Total Liabilities & Equity</span>
                  <span className="font-mono">{(reportData?.totalLiabilities + reportData?.totalEquity).toLocaleString()}</span>
                </div>
              </div>
            </div>
          </div>
