...
export const InventoryModule: React.FC = () => {
  const [items, setItems] = useState<InventoryItem[]>([
    { sku: 'SKU-001', name: 'Standard Consumable A', category: 'General Supplies', quantityOnHand: 50, reorderPoint: 10, unitCost: 150000, sellingPrice: 200000, valuationMethod: 'FIFO', assetAccount: '1200 - Inventory Assets' },
    { sku: 'SKU-002', name: 'Institutional Asset B', category: 'Equipment', quantityOnHand: 5, reorderPoint: 2, unitCost: 1200000, sellingPrice: 1500000, valuationMethod: 'FIFO', assetAccount: '1200 - Inventory Assets' }
  ]);
    ...
    <div className="space-y-6 animate-in fade-in duration-300 pb-16">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">Sovereign FAAP-compliant inventory valuation (FIFO/AVCO), stock reorder alerts & sales-linked ledger posting.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-3xl font-black text-slate-900 mt-1 font-mono">{formatCurrency(totalAssetValuation)}</p>
            ...
            {items.filter(i => i.name.toLowerCase().includes(search.toLowerCase()) || i.sku.toLowerCase().includes(search.toLowerCase())).map((item) => {
              ...
              return (
                <tr key={item.sku} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right font-mono font-bold text-slate-900">{formatNumeric(item.quantityOnHand)}</td>
                  <td className="px-6 py-4 text-right font-mono text-slate-700">{formatNumeric(item.unitCost)}</td>
                  <td className="px-6 py-4 text-right font-mono font-black text-emerald-700">{formatNumeric(totalVal)}</td>
