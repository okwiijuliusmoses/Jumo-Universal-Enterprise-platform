    ...
    <div className="space-y-6 animate-in fade-in duration-300 pb-16">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">QuickBooks-benchmarked project costing, labor & material cost allocation, milestone billing & project profit margins.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-3xl font-black text-slate-900 mt-1 font-mono">{totalBudget.toLocaleString()} UGX</p>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Incurred Work-in-Progress (WIP)</span>
          <p className="text-3xl font-black text-rose-600 mt-1 font-mono">{totalCost.toLocaleString()} UGX</p>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Billed Revenue Recognized</span>
          <p className="text-3xl font-black text-emerald-700 mt-1 font-mono">{totalRev.toLocaleString()} UGX</p>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {projects.map((p) => (
              <tr key={p.code} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4 text-right font-mono text-slate-700">{p.budget.toLocaleString()}</td>
                <td className="px-6 py-4 text-right font-mono text-rose-600 font-bold">{p.incurredCost.toLocaleString()}</td>
                <td className="px-6 py-4 text-right font-mono text-emerald-700 font-bold">{p.billedRevenue.toLocaleString()}</td>
