...
- Debit Salary Expense (5010): ${totalGross.toLocaleString()} UGX
- Debit NSSF Employer Expense (5015): ${totalNssfEmployer.toLocaleString()} UGX
- Credit PAYE Tax Payable (2030): ${totalPaye.toLocaleString()} UGX
- Credit NSSF Payable (2035): ${(totalNssfEmployee + totalNssfEmployer).toLocaleString()} UGX
- Credit Net Salaries Payable / Bank (1010): ${totalNet.toLocaleString()} UGX
    ...
    <div className="space-y-6 animate-in fade-in duration-300 pb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-2xl font-black text-slate-900 mt-1 font-mono">{totalGross.toLocaleString()} UGX</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">URA PAYE Income Tax</span>
          <p className="text-2xl font-black text-rose-600 mt-1 font-mono">{totalPaye.toLocaleString()} UGX</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total NSSF (15% Fund)</span>
          <p className="text-2xl font-black text-blue-600 mt-1 font-mono">{(totalNssfEmployee + totalNssfEmployer).toLocaleString()} UGX</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Net Salary Disbursement</span>
          <p className="text-2xl font-black text-emerald-700 mt-1 font-mono">{totalNet.toLocaleString()} UGX</p>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {employees.map((emp) => (
              <tr key={emp.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4 text-right font-mono font-bold text-slate-900">{emp.grossSalary.toLocaleString()}</td>
                <td className="px-6 py-4 text-right font-mono text-slate-600">{emp.nssfEmployee.toLocaleString()}</td>
                <td className="px-6 py-4 text-right font-mono text-rose-600 font-bold">{emp.payeTax.toLocaleString()}</td>
                <td className="px-6 py-4 text-right font-mono font-black text-emerald-700">{emp.netSalary.toLocaleString()}</td>
