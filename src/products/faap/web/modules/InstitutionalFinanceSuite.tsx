...
export const InstitutionalFinanceSuite: React.FC<{ initialSubView?: SubView }> = ({ initialSubView = 'VOTE_BOOK' }) => {
  ...
  const handleCommitEncumbrance = (e: React.FormEvent) => {
    ...
    if (requisitionAmount > available) {
      alert(`PRE-EXPENDITURE VOTE BOOK BLOCK: Requisition of ${requisitionAmount.toLocaleString()} UGX exceeds remaining available budget (${available.toLocaleString()} UGX) for vote ${vote.code}. Requisition rejected.`);
      return;
    }

    setVotes(prev => prev.map(v => v.code === requisitionVoteCode ? { ...v, encumbered: v.encumbered + requisitionAmount } : v));
    alert(`ENCUMBRANCE APPROVED: ${requisitionAmount.toLocaleString()} UGX committed against ${vote.code}. Local Purchase Order (LPO) clearance token generated.`);
    setRequisitionDesc('');
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
              ...
              <p className="text-2xl font-black text-slate-900 mt-1 font-mono">
                {votes.reduce((acc, v) => acc + v.approvedBudget, 0).toLocaleString()} UGX
              </p>
            </div>
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
              ...
              <p className="text-2xl font-black text-amber-600 mt-1 font-mono">
                {votes.reduce((acc, v) => acc + v.encumbered, 0).toLocaleString()} UGX
              </p>
            </div>
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
              ...
              <p className="text-2xl font-black text-rose-600 mt-1 font-mono">
                {votes.reduce((acc, v) => acc + v.actualSpent, 0).toLocaleString()} UGX
              </p>
            </div>
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
              ...
              <p className="text-2xl font-black text-emerald-600 mt-1 font-mono">
                {votes.reduce((acc, v) => acc + (v.approvedBudget - v.encumbered - v.actualSpent), 0).toLocaleString()} UGX
              </p>
            </div>
          </div>
                  ...
                  {votes.map((v) => {
                    ...
                    return (
                      <tr key={v.code} className="hover:bg-slate-50/50">
                        ...
                        <td className="px-6 py-4 text-right font-mono font-bold text-slate-900">{v.approvedBudget.toLocaleString()} UGX</td>
                        <td className="px-6 py-4 text-right font-mono font-bold text-amber-600">{v.encumbered.toLocaleString()} UGX</td>
                        <td className="px-6 py-4 text-right font-mono font-bold text-rose-600">{v.actualSpent.toLocaleString()} UGX</td>
                        <td className="px-6 py-4 text-right font-mono font-bold text-emerald-600">{available.toLocaleString()} UGX</td>
              ...
              <div key={v.code} className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-3">
                ...
                <div className="space-y-1 text-xs">
                  <div className="flex justify-between text-slate-500">
                    ...
                    <span className="font-mono font-bold text-slate-900">{v.approvedBudget.toLocaleString()} UGX</span>
                  </div>
                  <div className="flex justify-between text-slate-500">
                    <span>Encumbered (Committed):</span>
                    <span className="font-mono font-bold text-amber-600">{v.encumbered.toLocaleString()} UGX</span>
                  </div>
                  <div className="flex justify-between text-slate-500">
                    <span>Actual Disbursed:</span>
                    <span className="font-mono font-bold text-rose-600">{v.actualSpent.toLocaleString()} UGX</span>
                  </div>
                </div>
                <div className="pt-2 border-t border-slate-200 flex justify-between items-center text-xs font-bold">
                  ...
                  <span className="font-mono text-emerald-600">{(v.approvedBudget - v.encumbered - v.actualSpent).toLocaleString()} UGX</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden space-y-4 p-6">
          <div className="flex items-center justify-between">
            ...
            <span className="px-3 py-1 bg-emerald-100 text-emerald-800 text-xs font-bold rounded-full">
              Cash Closing Balance: {cashEntries.reduce((acc, e) => acc + (e.cashDebit - e.cashCredit), 0).toLocaleString()} UGX
            </span>
          </div>
                ...
                {cashEntries.map((e, idx) => {
                  ...
                  return (
                    <tr key={e.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-right font-mono font-bold text-emerald-600">{e.cashDebit > 0 ? e.cashDebit.toLocaleString() : '-'}</td>
                      <td className="px-4 py-3 text-right font-mono font-bold text-rose-600">{e.cashCredit > 0 ? e.cashCredit.toLocaleString() : '-'}</td>
                      <td className="px-4 py-3 text-right font-mono font-bold text-slate-900">{runningCash.toLocaleString()} UGX</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden space-y-4 p-6">
          <div className="flex items-center justify-between">
            ...
            <span className="px-3 py-1 bg-blue-100 text-blue-800 text-xs font-bold rounded-full">
              Bank Balance: {cashEntries.reduce((acc, e) => acc + (e.bankDebit - e.bankCredit), 0).toLocaleString()} UGX
            </span>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {cashEntries.map((e) => (
                  <tr key={e.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3 font-mono text-xs">{e.date}</td>
                    <td className="px-4 py-3 font-bold text-slate-900">{e.description}</td>
                    <td className="px-4 py-3 text-right font-mono text-emerald-600">{e.cashDebit > 0 ? e.cashDebit.toLocaleString() : '-'}</td>
                    <td className="px-4 py-3 text-right font-mono text-rose-600">{e.cashCredit > 0 ? e.cashCredit.toLocaleString() : '-'}</td>
                    <td className="px-4 py-3 text-right font-mono text-emerald-600">{e.bankDebit > 0 ? e.bankDebit.toLocaleString() : '-'}</td>
                    <td className="px-4 py-3 text-right font-mono text-rose-600">{e.bankCredit > 0 ? e.bankCredit.toLocaleString() : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody className="divide-y divide-slate-100">
                {cashEntries.map((e) => (
                  <tr key={e.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 font-bold text-slate-900">{e.description}</td>
                    <td className="px-4 py-3 text-right font-mono text-amber-600 font-bold">{e.discountAllowed > 0 ? e.discountAllowed.toLocaleString() : '-'}</td>
                    <td className="px-4 py-3 text-right font-mono text-emerald-600 font-bold">{e.cashDebit > 0 ? e.cashDebit.toLocaleString() : '-'}</td>
                    <td className="px-4 py-3 text-right font-mono text-emerald-600 font-bold">{e.bankDebit > 0 ? e.bankDebit.toLocaleString() : '-'}</td>
                    <td className="px-4 py-3 text-right font-mono text-purple-600 font-bold">{e.discountReceived > 0 ? e.discountReceived.toLocaleString() : '-'}</td>
                    <td className="px-4 py-3 text-right font-mono text-rose-600 font-bold">{e.cashCredit > 0 ? e.cashCredit.toLocaleString() : '-'}</td>
                    <td className="px-4 py-3 text-right font-mono text-rose-600 font-bold">{e.bankCredit > 0 ? e.bankCredit.toLocaleString() : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
