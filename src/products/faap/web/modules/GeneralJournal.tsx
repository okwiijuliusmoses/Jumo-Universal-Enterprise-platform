...
export const GeneralJournal: React.FC = () => {
  ...
  const columns: Column<FaapJournalEntry>[] = [
    ...
    { 
      header: 'AMOUNT (UGX)', 
      accessor: (j) => <span className="font-mono font-black text-slate-900">{formatNumber(j.totalDebit)}</span>,
              ...
              <>
                ...
                <div className="w-64 bg-white border border-slate-200 rounded-xl overflow-hidden self-start">
                  <div className="px-4 py-2 border-b border-slate-100 flex justify-between text-sm">
                    ...
                    <span className="font-mono font-bold text-slate-900">{formatNumber(totalDebits)}</span>
                  </div>
                  <div className="px-4 py-2 border-b border-slate-100 flex justify-between text-sm">
                    <span className="text-slate-500 font-bold">Total Credits</span>
                    <span className="font-mono font-bold text-slate-900">{formatNumber(totalCredits)}</span>
                  </div>
                  <div className={`px-4 py-2 flex justify-between text-sm font-bold ${difference === 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
                    ...
                    <span className="font-mono">{formatNumber(difference)}</span>
                  </div>
                </div>
              </>
            )
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                ...
                <p className="text-lg font-black text-slate-900 font-mono">{formatMoney(auditReport.totalAssets, 'UGX')}</p>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-[10px] font-bold text-slate-400 uppercase">Total Liabilities (CR)</p>
                <p className="text-lg font-black text-slate-900 font-mono">{formatMoney(auditReport.totalLiabilities, 'UGX')}</p>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-[10px] font-bold text-slate-400 uppercase">Total Equity (CR)</p>
                <p className="text-lg font-black text-slate-900 font-mono">{formatMoney(auditReport.totalEquity, 'UGX')}</p>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                ...
                <p className={`text-lg font-black font-mono ${auditReport.trialBalanceOffset === 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                  {formatMoney(auditReport.trialBalanceOffset, 'UGX')}
                </p>
              </div>
            </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {auditReport.accountAudits.map((audit) => (
                      <tr key={audit.accountCode} className={audit.hasDiscrepancy ? 'bg-rose-50' : ''}>
                        ...
                        <td className="px-4 py-2.5 text-right font-mono font-medium">{formatMoney(audit.cachedBalance, '')}</td>
                        <td className="px-4 py-2.5 text-right font-mono font-medium">{formatMoney(audit.calculatedBalance, '')}</td>
