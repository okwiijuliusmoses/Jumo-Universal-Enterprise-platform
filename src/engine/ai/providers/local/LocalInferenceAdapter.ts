...
export class LocalInferenceAdapter {
  ...
  public resolveConfiguredEndpoint(): string {
    ...
    if (configured) {
      ...
      this.candidateEndpoints = [configured, ...defaultCandidates.filter(c => c !== configured)];
    } else {
      this.primaryEndpoint = "http://127.0.0.1:11434";
      this.candidateEndpoints = defaultCandidates;
    }
  ...
  public async discoverRuntime(): Promise<{ reachable: boolean; endpoint: string; state: LocalRuntimeState }> {
    ...
    for (const ep of this.candidateEndpoints) {
      try {
        ...
        if (!ep) continue;
        const ollaUrl = JumoAIUrlResolver.resolve(ep, "/olla/models");
        ...
        if (!ep) continue;
        const tagsUrl = JumoAIUrlResolver.resolve(ep, "/api/tags");
  ...
  public async discoverModels(customEndpoint?: string): Promise<LocalDiscoveredModel[]> {
    ...
    for (const path of probePaths) {
      try {
        ...
        const url = JumoAIUrlResolver.resolve(targetEndpoint, path);
        const controller = new AbortController();
  ...
  public async checkInferenceHealth(): Promise<LocalInferenceHealthResult> {
    ...
    for (const path of testPaths) {
      try {
        if (!discovery.endpoint) continue;
        const url = JumoAIUrlResolver.resolve(discovery.endpoint, path);
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), 2000);
  ...
  public async executeInference(
    ...
    for (const path of targetPaths) {
      try {
        ...
        const url = JumoAIUrlResolver.resolve(endpoint, path);
        const controller = new AbortController();
    ...
    try {
      ...
      const url = JumoAIUrlResolver.resolve(endpoint, "/v1/chat/completions");
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), 10000);
      const res = await fetch(url, {
        method: "POST",
