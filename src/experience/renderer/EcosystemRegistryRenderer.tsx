...
export function EcosystemRegistryRenderer() {
  ...
  const displayEcosystems = Array.isArray(ecosystems) ? ecosystems : [];

  const filteredEcosystems = displayEcosystems.filter(e => {
    const nameStr = String(e?.name ?? "").toLowerCase();
    const authStr = String(e?.authority ?? "").toLowerCase();
    const regIdStr = String(e?.registryId ?? e?.id ?? "").toLowerCase();
    const searchStr = (search || "").toLowerCase();
    return nameStr.includes(searchStr) || authStr.includes(searchStr) || regIdStr.includes(searchStr);
  });
