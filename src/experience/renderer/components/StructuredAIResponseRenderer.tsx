...
export function StructuredAIResponseRenderer({
  ...
  return (
    <div className={`space-y-3.5 font-sans ${className}`}>
      ...
      {mainText !== undefined && (
        <div className={`leading-relaxed text-xs font-medium space-y-2 ${isDark ? "text-slate-200" : "text-slate-800"}`}>
          {safeText(mainText).split('\n\n').map((paragraph: string, pIdx: number) => (
      ...
      {mainText === undefined && !plan && !delegation && (
        <div className={`p-3 rounded-lg border text-xs font-medium leading-relaxed ${isDark ? "bg-slate-900/90 text-slate-300 border-slate-800" : "bg-slate-50 text-slate-800 border-slate-200"}`}>
          {safeText(response)}
        </div>
      )}
