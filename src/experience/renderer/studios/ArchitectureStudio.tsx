...
export const ArchitectureStudio: React.FC<ArchitectureStudioProps> = ({
  ...
  const [primaryTab, setPrimaryTab] = useState<'contracts' | 'explorer' | 'workforce'>('explorer');
            ...
            <div className="bg-slate-900 border border-slate-800 text-white rounded-2xl p-5 shadow-xl space-y-4">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <h3 className="text-sm font-black text-white uppercase mt-0.5 tracking-tight flex items-center gap-2">
                      <span>JUMO GPT INTELLIGENCE STATUS: {providerHealth?.intelligenceStatus || "OPERATIONAL"}</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs">
                {[
                  ...
                  const healthInfo = providerHealth?.providers?.find((ph: any) => ph.providerId === p.id);
                  const isHealthy = healthInfo ? healthInfo.status === "HEALTHY" : true; // default true if not polled yet
                  const latency = healthInfo?.latencyMs ? `${healthInfo.latencyMs}ms` : "120ms";
                  const modeStr = healthInfo?.details || "Autonomous Direct Bridge";
                  ...
                  return (
                    <div key={p.id} className="bg-slate-950 border border-slate-800/80 rounded-xl p-3.5 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`px-1.5 py-0.2 rounded font-mono text-[8px] font-extrabold ${isHealthy ? 'bg-emerald-950 text-emerald-400' : 'bg-rose-950 text-rose-400'}`}>
