...
export const RuntimeOperationsStudio: React.FC<OperationsStudioProps> = ({ runtimeInstances = [] }) => {
  const [activeTab, setActiveTab] = useState<'telemetry' | 'maintenance' | 'incidents' | 'backups' | 'upgrades' | 'ai-control'>('telemetry');
    ...
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">Institutional Operations & Maintenance Studio</h2>
            <p className="text-sm text-slate-500 font-medium">JDPM-4000 Live Institutional Telemetry, AI Incident Recovery & Upgrades</p>
          </div>
        </div>

        <div className="flex items-center gap-2 p-1 bg-slate-100 rounded-lg flex-wrap">
          {(['telemetry', 'maintenance', 'incidents', 'backups', 'upgrades', 'ai-control'] as const).map(tab => (
            <button
              key={tab}
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
                ...
                <div className="divide-y divide-slate-100">
                  {maintenanceTasks.map(task => (
                    <div key={task.taskId} className="py-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="text-[10px] text-slate-500 font-mono">
                          Target: {task.installationId} • Scheduled: {new Date(task.scheduledTime).toLocaleString()} • Auth: {task.authorizedBy}
                        </div>
                        {task.resultLog && (
                          <div className="text-[10px] text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded font-mono">
                            {task.resultLog}
                          </div>
                        )}
                      </div>
            ...
            >
              ...
              <div className="lg:col-span-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
                ...
                <form onSubmit={handleRaiseIncident} className="space-y-3">
                  ...
                  <button
                    type="submit"
                    className="w-full py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-bold uppercase"
                  >
                    Raise Incident & Trigger AI Diagnosis
                  </button>
                </form>
              </div>
            </motion.div>
                ...
                <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                  <div>
                    <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                      ...
                      Disaster Recovery & Encrypted Snapshots ({backups.length})
                    </h3>
                    <p className="text-xs text-slate-500 font-medium">All snapshots verified with SHA-256 cryptographic integrity hashes.</p>
                  </div>
                  <button
                    onClick={handleCreateBackup}
                    className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold flex items-center gap-1.5"
                  >
                    <Archive size={14} /> Create Snapshot Now
                  </button>
                </div>

                <div className="divide-y divide-slate-100">
                  {backups.map(bkp => (
                    <div key={bkp.backupId} className="py-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-xs text-slate-900">{bkp.backupId}</span>
                          <span className="text-[9px] font-mono bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded">{bkp.backupType}</span>
                        </div>
                        <div className="text-[10px] text-slate-500 font-mono">
                          Size: {bkp.sizeMb} MB • Encryption: {bkp.encryptedWith} • Created: {new Date(bkp.createdAt).toLocaleString()}
                        </div>
                        <div className="text-[9px] text-slate-400 font-mono truncate max-w-xl">
                          Digest: {bkp.sha256Digest}
                        </div>
                      </div>
          ...
          {activeTab === 'upgrades' && (
            ...
            >
              <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
                <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                  <div>
                    <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                      ...
                      Institutional Upgrade Engine ({upgrades.length})
                    </h3>
                    <p className="text-xs text-slate-500 font-medium">Zero-downtime blue-green upgrades with pre/post-test verification and automatic rollback.</p>
                  </div>
                  <button
                    onClick={handlePlanUpgrade}
                ...
                <div className="divide-y divide-slate-100">
                  {upgrades.map(upg => (
                    <div key={upg.upgradeId} className="py-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-xs text-slate-900">Version {upg.fromVersion} → {upg.toVersion}</span>
                          <span className="text-[9px] font-mono bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded">{upg.upgradeId}</span>
                        </div>
                        <div className="text-[10px] text-slate-500 font-mono">
                          Pre-Backup Ref: {upg.preUpgradeBackupId} • Estimated Downtime: {upg.impactAnalysis.downtimeEstimateSeconds}s
                        </div>
                        <div className="text-[10px] text-slate-600">
                          Breaking Changes: {upg.impactAnalysis.breakingChanges.join(', ') || 'None'}
                        </div>
                      </div>
                  ...
                  {upgrades.length === 0 && (
                    <div className="text-center py-12 text-slate-400 text-xs italic">No active upgrade plans. Click "Plan v1.1.0 Upgrade" to initiate.</div>
                  )}
                </div>
              </div>
            </motion.div>
            ...
            >
              <div>
                <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                  ...
                  JUMO GPT Operational Control Console
                </h3>
              ...
              <form onSubmit={handleRunGptQuery} className="flex gap-2">
                <input
                  ...
                  placeholder="Ask JUMO GPT an authorized operational query..."
                  className="flex-1 p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-800"
                />
                <button
                  type="submit"
                  disabled={gptLoading}
                  className="px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold flex items-center gap-2"
                >
              ...
              {gptResponse && (
                <div className="p-5 bg-slate-900 text-slate-200 rounded-2xl border border-slate-800 space-y-2">
                  <div className="text-[10px] font-black uppercase text-blue-400 tracking-wider flex items-center gap-1.5">
                    <CheckSquare size={12} /> JUMO GPT Authoritative Response
                  </div>
                  <div className="text-xs font-mono leading-relaxed">{gptResponse}</div>
                </div>
              )}
            </motion.div>
          )}
