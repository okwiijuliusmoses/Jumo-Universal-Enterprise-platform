      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 text-slate-700">
          ...
          <div className="space-y-5 animate-in fade-in-50 duration-150">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-violet-100 text-violet-800 flex items-center justify-center text-sm font-black uppercase border border-violet-200">
                {data.displayName?.substring(0, 2)}
              </div>
              <div>
        ...
        {type === "job" && (
          <div className="space-y-5 animate-in fade-in-50 duration-150">
            ...
            <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl space-y-2 text-[11px]">
              <div className="flex justify-between border-b border-slate-200/50 pb-1.5">
                <span className="text-slate-400 font-bold">Job ID:</span>
                <span className="font-extrabold text-slate-800 font-mono">{data.id}</span>
              </div>
              <div className="flex justify-between border-b border-slate-200/50 pb-1.5">
                <span className="text-slate-400 font-bold">Type:</span>
                <span className="font-semibold text-slate-800">{data.type}</span>
              ...
              <div className="flex justify-between border-b border-slate-200/50 pb-1.5">
                <span className="text-slate-400 font-bold">Blueprint ID:</span>
                <span className="font-semibold text-slate-800 font-mono">{data.blueprintId}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400 font-bold">Current State:</span>
                <span className="font-extrabold text-blue-600">{data.status}</span>
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                <div className="bg-blue-600 h-1.5 rounded-full" style={{ width: `${data.progress}%` }}></div>
              </div>
            </div>

            {data.logs && (
              <div className="space-y-2">
                <span className="text-[10px] font-black uppercase text-slate-400">Stream Logs</span>
                <div className="bg-slate-950 font-mono text-[10px] text-emerald-400 border border-slate-800 p-3 rounded-xl space-y-1 h-44 overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => onTriggerAction?.("pause-job", data)}
                className="py-2 border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-xl text-xs font-bold cursor-pointer"
              >
                Pause Job
              </button>
              <button
                onClick={() => onTriggerAction?.("promote-job", data)}
                className="py-2 bg-slate-900 hover:bg-blue-600 text-white rounded-xl text-xs font-bold cursor-pointer"
              >
                Promote Release
              </button>
            </div>
