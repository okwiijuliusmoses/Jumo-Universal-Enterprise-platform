...
export class SovereignOperatingStateService {
  ...
  private static getInitialState(): SovereignState {
    ...
    return {
      ...
      aiGateway: {
        registeredProviders: [
          {
            ...
            supportedModels: ["gemini-3.7-flash", "gemini-3.1-pro-preview", "gemini-3.1-flash-lite"],
            isAvailable: true
          },
          {
            providerId: "openai",
        ...
        activePrimaryProvider: "openai",
        fallbackProviderOrder: ["openai", "gemini", "copilot", "jumo_local"],
  ...
  public static createArchitectureContract(reqId: string, actor: string) {
    ...
    const newContract: ArchitectureContract = {
      ...
      domainArchitecture: {
        domainIdentifier: req.ecosystemType,
        coreWorkflows: req.capabilities,
      ...
      manufacturingDirectives: {
        ...
        priorityModules: req.capabilities,
        integrationTargets: req.integrations
      },
  ...
  public static createManufacturingJob(contractId: string, actor: string) {
    ...
    const newJob: ManufacturingJob = {
      ...
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      name: `Job for ${contract.productIdentity.name}`,
      description: `Authoritative manufacturing job for ${contract.productIdentity.name}`,
      layerIds: [],
      mandatoryLayerIds: [],
      config: {}
    };
  ...
  public static launchPipelineFromBlueprint(bpId: string, actor: string) {
    ...
    const newJob: ManufacturingJob = {
      ...
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      name: `Job for ${bp.name}`,
      description: `Authoritative manufacturing job for ${bp.name}`,
      layerIds: [],
      mandatoryLayerIds: [],
      config: {}
    };
  ...
  public static recordDeployment(jobId: string, env: DeploymentRecord['environment'], target: string, actor: string) {
    ...
    const newRecord: DeploymentRecord = {
      ...
      healthCheck: 'PASSED',
      timestamp: new Date().toISOString(),
      logs: [`[DEPLOY] Deployed to ${env} at ${target}`]
    };
  ...
  public static recordVerificationFailure(jobId: string, layerId: string, diagnostic: string, actor: string) {
    ...
    const newFailure: VerificationFailureRecord = {
      ...
      jobId,
      layerId,
      stage: job.currentLifecycleState,
      reason: "Architecture Conformance Failure",
  ...
  public static certifyManufacturingJob(jobId: string, authority: string, actor: string) {
    ...
    const newCert: CertificationRecord = {
      ...
      authority: authority,
      seal: `SEAL-${Date.now().toString(36).toUpperCase()}`,
      evidence: job.evidence.map(e => ({ type: 'STATE_EVIDENCE', hash: e })),
      timestamp: new Date().toISOString()
    };
