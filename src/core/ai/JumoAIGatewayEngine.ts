...
export class JumoAIGatewayEngine {
  ...
  static async getAIFabricHealth(): Promise<{
    ...
    const isRuntimeHealthy = adapterHealth.status === 'HEALTHY' || adapterHealth.status === 'DEGRADED';
    const isProviderHealthy = providerHealth.status === 'HEALTHY' || providerHealth.status === 'DEGRADED';
