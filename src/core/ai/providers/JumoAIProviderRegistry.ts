...
export class JumoAIProviderRegistry {
  ...
  register(provider: JumoAIProvider): void {
    ...
    // Store under original ID and canonical key
  ...
  get(providerId: string): JumoAIProvider {
    ...
    // Check direct map first (exact ID or canonical)
    const direct = this.providers.get(providerId) || this.providers.get(normalized);
  ...
  list(): JumoAIProvider[] {
    // Return unique provider instances
