...
export class AnthropicClaudeProvider implements JumoAIProvider {
  ...
  async isAvailable(): Promise<boolean> {
    const hasKey = !!JumoSecretVault.getInstance().getAnthropicKey();
  ...
  async getHealth(): Promise<{ status: "HEALTHY" | "DEGRADED" | "UNAVAILABLE"; latencyMs?: number; details?: string }> {
    const apiKey = JumoSecretVault.getInstance().getAnthropicKey();
    if (!apiKey || apiKey.trim().length === 0) {
      return {
        status: "UNAVAILABLE",
        details: "Anthropic API Key (JUMO_ANTHROPIC_API_KEY) not configured in JumoSecretVault."
      };
    }

    const startTime = Date.now();
    try {
      const url = JumoAIUrlResolver.resolve("https://api.anthropic.com", "/v1/models");
  ...
  async generate(request: JumoAIRequest): Promise<JumoAIResponse> {
    ...
    const url = JumoAIUrlResolver.resolve("https://api.anthropic.com", "/v1/messages");
