...
export class AgentExecutionService {
   ...
   */
  public static async executeAgentTask(request: AgentTaskRequest, actor: string): Promise<AgentWorkLog> {
    ...
    const aiResult = await gateway.executeAgentTask(
      agent,
      request.task,
      request.task, // Use task as prompt if no separate prompt provided
      { jobId: request.jobId, architectureId: request.architectureId }
    );
    ...
    SovereignOperatingStateService.logAudit(actor, "AGENT_TASK_EXECUTED", `Agent ${agent.jumoName} ${aiResult.success ? 'completed' : 'failed'} task: ${request.task}`);
    SovereignOperatingStateService.emitEvent({
      sourceStudio: "ENGINEERING",
      destinationStudio: "ALL",
      entityId: workLog.jobId,
      action: "AGENT_TASK_EXECUTED",
      status: workLog.status === 'COMPLETED' ? 'EXECUTED' : 'FAILED',
      payload: { agentId: request.agentId, task: request.task }
    });
