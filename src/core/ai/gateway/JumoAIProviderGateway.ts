...
- **Verification Hash:** Verified against local baseline.`;
  ...
  public async reasoning(request: { message: string; systemPrompt?: string; modelId?: string; temperature?: number; context?: Record<string, any>; providerId?: string }): Promise<{ text: string; modelId: string; providerId: string }> {
    ...
    let targetProviderId = request.providerId ? registry.get(request.providerId).providerId : undefined;
    ...
    try {
      ...
      if (targetProviderId !== "JUMO_LOCAL") {
        ...
        if (isLocalErrorState) {
           const finalError = `AI_EXECUTION_UNAVAILABLE: [CRITICAL_AI_FAILURE] All providers (Primary: ${targetProviderId} and Local Fallback) failed. Cause: ${localResult.text || localResult.metadata?.error || "Unknown"}`;
