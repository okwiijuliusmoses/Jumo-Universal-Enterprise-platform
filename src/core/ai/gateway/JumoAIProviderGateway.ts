...
export class JumoAIProviderGateway {
  ...
  public async executeAgentTask(
    ...
    const availableProviders = await registry.available();
    
    trace.push(`[GATEWAY] Detected ${availableProviders.length} active sovereign intelligence providers.`);
    ...
    if (config.mode === "AIR-GAP") {
      ...
      const hasPreferred = availableProviders.find(p => p.providerId === preferred);
      ...
      if (hasPreferred) {
        ...
        const fallbacks = availableProviders.filter(p => !p.local);
        if (fallbacks.length > 0) {
          targetProviderId = fallbacks[0].providerId;
          trace.push(`[GATEWAY] Preferred provider unavailable. Falling back to: ${targetProviderId}`);
        } else {
          targetProviderId = "JUMO_LOCAL";
          trace.push(`[GATEWAY] No remote providers available. Using local reasoning.`);
        }
      }
    }
