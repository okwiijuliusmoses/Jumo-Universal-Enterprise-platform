...
export class JumoLocalInferenceEngine implements LocalInferenceRuntime {
  ...
  async healthCheck(): Promise<{ status: 'HEALTHY' | 'UNHEALTHY' | 'OFFLINE'; latencyMs: number; details: string; state: LocalExecutionProgressState }> {
    ...
    // 0. Check Local Express / Olla Server Proxy / native Ollama daemon
    const probeEndpoints = [
      process.env.JUMO_SOVEREIGN_INFERENCE_ENDPOINT,
    ...
    for (const ep of probeEndpoints) {
      const paths = ep.includes('11434') ? ['/api/tags'] : 
                    (ep.includes('8000') ? ['/v1/models'] : 
                    (ep.includes('8080') ? ['/health'] : ['/olla/models', '/api/tags']));

      for (const path of paths) {
        const url = JumoAIUrlResolver.resolve(ep, path);
        if (await probeJson(url)) {
          this.endpointUrl = ep;
          if (ep.includes('11434')) this.engineType = 'OLLAMA';
          else if (ep.includes('8000')) this.endpointUrl = `${ep}/v1`, this.engineType = 'VLLM';
          else if (ep.includes('8080')) this.engineType = 'LLAMA_CPP';
          else this.engineType = 'OLLA_PROXY';
          ...
          if (models.length > 0) {
            ...
            const testPing = await this.generate('ping', { modelId: testModel });
            if (testPing.success) {
              this.executionState = 'LOCAL_AI_READY';
              return {
                status: 'HEALTHY',
                latencyMs: testPing.latencyMs,
                details: `${this.engineType} Local AI Engine active on ${this.endpointUrl}. Tested '${testModel}' in ${testPing.latencyMs}ms.`,
          ...
          return {
            ...
            details: `${this.engineType} local inference active on ${this.endpointUrl}`,
  ...
  async discoverModels(): Promise<Array<{ modelId: string; displayName: string; contextLength: number; capabilities: string[] }>> {
    ...
    if (this.engineType === 'OLLA_PROXY' || this.engineType === 'OLLAMA') {
      ...
      try {
        if (!this.endpointUrl) return discovered;
        const url = JumoAIUrlResolver.resolve(this.endpointUrl, "/models");
  ...
  async generate(prompt: string, options?: LocalInferenceOptions): Promise<LocalInferenceResult> {
    ...
    try {
      if (this.engineType === 'OLLA_PROXY' || this.engineType === 'OLLAMA') {
        const url = JumoAIUrlResolver.resolve(this.endpointUrl, "/api/generate");
        const res = await fetch(url, {
          method: 'POST',
        ...
        const url = JumoAIUrlResolver.resolve(this.endpointUrl, "/chat/completions");
        const res = await fetch(url, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
