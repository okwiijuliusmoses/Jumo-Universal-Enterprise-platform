...
export class JumoMaintenanceManufacturingPipeline {
   ...
   */
  static async executePipeline(component: string, errorMessage: string): Promise<AutonomousMaintenanceSession> {
    ...
    await JumoAutonomousMaintenanceEngine.runRootCauseAnalysis(session.id);
