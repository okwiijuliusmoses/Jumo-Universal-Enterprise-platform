...
import { 
  ...
  TrendingUp, Send, CheckCircle, RefreshCw, Clipboard, FileSpreadsheet, Lock, LogIn, UserPlus
} from 'lucide-react';
import { PlatformSwitcher } from '../../../components/PlatformSwitcher';
import { EducationErpService } from '../domain/EducationErpService';
import { EducationTemplateRegistry, PortalRegistry, calculateRegistryStats, WorkflowRegistry, ModuleRegistry } from '../../registries';
...
export const EducationErpWebShell: React.FC<{ onNavigate?: (route: string) => void }> = ({ onNavigate }) => {
  ...
  const [activePortalId, setActivePortalId] = useState<string>('EDU_EXECUTIVE');
  ...
  const renderLoginPage = () => (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-lg max-w-md w-full">
        ...
        <p className="text-center text-sm text-slate-500 mb-8">Authenticate into JUMO Education ERP</p>
        ...
        <form onSubmit={(e) => { e.preventDefault(); setAppState('APP'); }} className="space-y-4">
          <div>
            ...
            >
              {PortalRegistry.filter(p => p.id.startsWith('EDU_')).map(p => (
  ...
  const renderCurrentPage = () => {
    switch (activeTab) {
      case 'DASHBOARD': return <EducationDashboard config={config} applications={applications} students={students} invoices={invoices} stats={stats} activePortalId={activePortalId} />;
      case 'GOVERNANCE': return <GovernanceModule />;
      case 'REGISTRAR': return <RegistrarModule students={students} />;
      case 'SENATE': return <SenateModule results={results} />;
      case 'BURSARY': return <BursaryModule invoices={invoices} voteBooks={voteBooks} />;
      case 'CLINIC': return <ClinicModule visits={clinicVisits} />;
      case 'LIBRARY': return <LibraryModule books={libraryBooks} />;
      case 'HOSTEL': return <HostelModule />;
      case 'SETTINGS': return (
        <div className="p-6">
          <h2 className="text-xl font-bold text-slate-800 mb-6">Institution Configuration</h2>
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm max-w-3xl">
            <h3 className="font-semibold text-slate-800 mb-4 border-b pb-2">Active Education Template</h3>
            <p className="text-sm text-slate-600 mb-4">Changing the template dynamically restructures all available departments, portals, and modules instantly.</p>
            <select 
            ...
            <h3 className="font-semibold text-slate-800 mb-4 border-b pb-2">Enabled Product Modules ({ModuleRegistry.filter(m => m.id.startsWith('MOD_EDU_')).length} available)</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {ModuleRegistry.filter(m => m.id.startsWith('MOD_EDU_')).slice(0, 24).map(mod => (
                <div key={mod.id} className="flex items-center gap-2">
                  <input type="checkbox" defaultChecked className="rounded text-blue-600" />
                  <span className="text-sm text-slate-700 truncate">{mod.displayName}</span>
                </div>
              ))}
              <div className="text-sm text-slate-500 italic mt-2">...and more derived from registry.</div>
            </div>
          </div>
        </div>
      );
      default: return <EducationDashboard config={config} applications={applications} students={students} invoices={invoices} stats={stats} activePortalId={activePortalId} />;
    }
  };
  ...
  const menuGroups = [
    {
      group: 'Portals',
      items: PortalRegistry.filter(p => p.id.startsWith('EDU_')).map(p => ({
        id: p.id,
        label: p.displayName,
        icon: UserCheck
      }))
    },
    {
      ...
      items: [
        ...
        { id: 'REGISTRAR', label: 'Registrar & Admissions', icon: Users },
        { id: 'SENATE', label: 'Senate & Academic Board', icon: Award },
        { id: 'BURSARY', label: 'Bursary & Finance', icon: Landmark },
        { id: 'GOVERNANCE', label: 'Governance & Directives', icon: Building }
      ]
    },
    {
      group: 'Student Services',
      items: [
        { id: 'CLINIC', label: 'Health Clinic', icon: HeartPulse },
        { id: 'LIBRARY', label: 'Library & Media', icon: BookOpen },
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={\`\${isSidebarOpen ? 'w-64' : 'w-0 hidden'} flex-shrink-0 bg-white border-r border-slate-200 flex flex-col overflow-y-auto transition-all duration-300 ease-in-out z-10\`}>
          <div className="p-4 border-b border-slate-100 bg-slate-50/50">
            ...
            <select 
              className="w-full text-sm bg-white border border-slate-200 rounded-md p-2 focus:ring-1 focus:ring-blue-500 focus:outline-none"
              value={activePortalId}
              onChange={(e) => handlePortalChange(e.target.value)}
            >
              {PortalRegistry.filter(p => p.id.startsWith('EDU_')).map(p => (
                ...
                <div className="space-y-0.5">
                  {group.items.map(item => {
                    ...
                    return (
                      ...
                      >
                        <Icon className={\`w-4 h-4 \${isSelected || (group.group === 'Portals' && activePortalId === item.id) ? 'text-blue-600' : 'text-slate-400'}\`} />
          ...
          <footer className="bg-white border-t border-slate-200 py-3 px-6 flex items-center justify-between text-[11px] text-slate-500 shrink-0">
            <div className="flex items-center gap-4">
              ...
              <span className="hidden sm:inline text-emerald-600 font-medium flex items-center gap-1">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div> All Ledgers Synchronized
              </span>
            </div>
            <div className="flex items-center gap-3">
...
`;
fs.writeFileSync('src/products/education-erp/web/EducationErpWebShell.tsx', shellContent);
