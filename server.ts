...
async function startServer() {
  ...
  // Ensure Ollama daemon is active if installed
  try {
    const { spawn, execSync } = eval('require("child_process")');
    let hasOllama = false;
    try {
      execSync("which ollama || test -f /usr/local/bin/ollama", { stdio: "ignore" });
      hasOllama = true;
    } catch {}

    if (hasOllama) {
      fetch("http://127.0.0.1:11434/api/tags").catch(() => {
        try {
          ...
          console.warn("[LocalInference] Ollama daemon start notice:", e);
        }
      });
    }
  } catch {}
  ...
  app.get("/olla/models", async (req, res) => {
    try {
      ...
      if (ollamaRes && ollamaRes.ok) {
        ...
        const models = (data.models || []).map((m: any) => ({
          ...
          displayName: `${m.name} (Sovereign Local)`,
          provider: "Ollama / Omalla",
          runtime: "Olla",
          details: m.details || {
            family: "llama",
            parameter_size: "0.5B",
    ...
    res.json([
      ...
      },
      {
        ...
        displayName: "Qwen 2.5 0.5B (Sovereign)",
        provider: "Ollama",
        runtime: "Olla",
        details: {
          family: "qwen2",
  ...
  app.post("/api/generate", async (req, res) => {
    ...
    // Forward real inference to local Ollama daemon
    try {
      const targetModel = model || "qwen2.5:0.5b";
      ...
      console.warn("[LocalInference] Ollama generate forwarding notice:", e);
    }

    if (prompt && prompt.toLowerCase().includes("ping")) {
      return res.json({ response: "pong", eval_count: 1 });
    }
  ...
  // JUMO UEOS AI Control Plane - Consolidated Route Registry
  ...
  app.post("/api/v1/ueos/ai/reasoning", async (req, res) => {
    try {
      ...
      const response = await JumoAIGatewayEngine.processReasoningRequest({
