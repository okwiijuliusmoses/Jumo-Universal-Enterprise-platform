...
export class ERPEcosystemTemplateRegistry {
      ...
      ecosystem.templates.forEach(template => {
        ...
        if (!this.templates.find(t => t.id === templateId)) {
          this.templates.push({
            ...
            departments: template.departments || [`${scopeName} Directorate`, `Finance Directorate`, `Operations Directorate`],
  ...
  loadFromStorage() {
    ...
    if (Array.isArray(stored)) {
      stored.forEach(t => {
        if (t && t.id && !this.templates.find(existing => existing.id === t.id)) {
