...
export function bootstrapEnterprisePlatform() {
  ...
  const existingERPs = erpRegistry.list();

  templates.forEach(template => {
    // Check if an instance of this template already exists in registry
    const alreadyRegistered = existingERPs.find(e => e.blueprintId === template.blueprintId || e.id === template.id);
    
    if (!alreadyRegistered) {
      console.log(`[UEOS] Auto-generating ERP Instance from Template: ${template.name}`);
      ...
      const erpDefinition = {
        ...
        portals: template.defaultPortals,
        modules: template.defaultModules,
        departments: template.defaultDepartments,
      ...
      try {
        ...
        console.error(`[UEOS] Failed to auto-generate ERP for template ${template.id}:`, err);
      }
    }
  });
