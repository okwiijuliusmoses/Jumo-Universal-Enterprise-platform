...
export const shellTemplate = (state) => {
  ...
  window.app.innerHTML = `
          <div class="min-h-screen flex flex-col bg-white font-sans text-slate-800">
            ...
            <header class="h-14 bg-white border-b border-slate-200 px-6 flex items-center justify-between sticky top-0 z-[60] shadow-sm">
              <div class="flex items-center space-x-6">
                ...
                <div class="relative group">
                  <button class="flex items-center gap-2 px-3 py-1.5 hover:bg-slate-100 rounded-md transition cursor-pointer">
                    ...
                    <span class="text-xs font-bold text-slate-700 uppercase tracking-wider">Portals</span>
                  </button>
                  <div class="absolute left-0 top-full mt-1 w-64 bg-white border border-slate-200 shadow-xl rounded-lg hidden group-hover:block z-50 p-2">
                    ...
                    ${state.session?.user?.isAdmin ? `
                      <button onclick="window.navigate('/control-center')" class="w-full text-left px-3 py-2 hover:bg-indigo-50 text-indigo-700 rounded text-xs font-bold transition">⚙️ Platform Control Center</button>
                    ` : ''}
                  </div>
                </div>
              ...
              <div class="flex items-center space-x-4">
                ...
                <div class="relative">
                  <button onclick="window.state.notificationCenterOpen = !window.state.notificationCenterOpen; window.render();" class="p-2 text-slate-500 hover:text-slate-800 transition relative cursor-pointer">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                    ${(state.notifications || []).length > 0 ? `<span class="absolute top-1 right-1 w-2 h-2 bg-rose-500 rounded-full border border-white"></span>` : ''}
                  </button>
                  ${window.state.notificationCenterOpen ? `
                    <div class="absolute right-0 mt-2 w-80 bg-white border border-slate-200 rounded-xl shadow-2xl p-4 z-50 divide-y divide-slate-100 text-xs">
                      <p class="font-bold text-slate-700 pb-2 uppercase tracking-wider">Notifications Center</p>
                ...
                <div class="h-8 w-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-700 font-bold text-xs">
                  ${(state.session?.user?.name || 'A').charAt(0)}
                </div>
              </div>
            </header>
            ...
            <div class="h-10 bg-slate-50 border-b border-slate-200 px-6 flex items-center text-[10px] font-bold uppercase tracking-wider text-slate-400">
               <div class="flex items-center gap-2">
                 ...
                 <span>/</span>
                 <span class="text-slate-900">${state.activeTab === 'launcher' ? 'Marketplace' : state.activeTab === 'domains' ? 'Sovereign Domains' : state.activeTab === 'workflows' ? 'Workflow Center' : state.activeTab === 'aegis' ? 'AEGIS Audit' : 'AI Assistant'}</span>
               </div>
            </div>
