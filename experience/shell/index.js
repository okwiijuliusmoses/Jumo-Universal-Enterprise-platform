  ...
  window.app.innerHTML = `
          <div class="min-h-screen flex bg-slate-50 font-sans">
            ...
            <aside class="w-64 bg-slate-900 text-slate-300 flex flex-col border-r border-slate-800 shrink-0">
              <div class="p-5 border-b border-slate-800 cursor-pointer" onclick="window.navigate('/gateway')">
                ${logoHtml}
              </div>
              ...
              <nav class="flex-1 p-3 space-y-1 text-xs">
                <button onclick="state.activeTab='launcher'; window.render();" class="w-full flex items-center space-x-3 px-3 py-2.5 rounded-lg font-bold uppercase tracking-wider text-left transition cursor-pointer ${state.activeTab==='launcher'?'bg-indigo-600 text-white':'hover:bg-slate-800 text-slate-400 hover:text-slate-200'}">
                  <span>🛍️ Application Marketplace</span>
                </button>
                <button onclick="state.activeTab='domains'; window.render();" class="w-full flex items-center space-x-3 px-3 py-2.5 rounded-lg font-bold uppercase tracking-wider text-left transition cursor-pointer ${state.activeTab==='domains'?'bg-indigo-600 text-white':'hover:bg-slate-800 text-slate-400 hover:text-slate-200'}">
                ...
                <button onclick="state.activeTab='ai'; window.render();" class="w-full flex items-center space-x-3 px-3 py-2.5 rounded-lg font-bold uppercase tracking-wider text-left transition cursor-pointer ${state.activeTab==='ai'?'bg-indigo-600 text-white':'hover:bg-slate-800 text-slate-400 hover:text-slate-200'}">
              ...
              <div class="p-3 border-t border-slate-800 space-y-1.5 text-xs">
                <div class="text-[9px] text-slate-500 font-mono mb-2">
                  Build: ${BUILD_INFO.commit}<br>
                  TS: ${BUILD_INFO.deploymentTimestamp}
                </div>
                <button onclick="window.navigate('/gateway')" class="w-full py-2 bg-slate-800 hover:bg-slate-700 text-white font-bold text-[10px] uppercase tracking-wider rounded-md text-center transition cursor-pointer">&larr; Personal Gateway</button>
                <button onclick="window.navigate('/workspace')" class="w-full py-2 bg-slate-800 hover:bg-slate-700 text-white font-bold text-[10px] uppercase tracking-wider rounded-md text-center transition cursor-pointer">Workspace Resolver</button>
            ...
            <div class="flex-1 flex flex-col min-w-0">
              ...
              <header class="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between shrink-0">
                ...
                <div class="flex items-center space-x-4">
                  <div class="relative">
                    <button onclick="window.state.notificationCenterOpen = !window.state.notificationCenterOpen; window.render();" class="p-2 text-slate-500 hover:text-slate-800 transition text-xs font-bold uppercase cursor-pointer">
                      🔔 Notifications (${(state.notifications || []).length})
                    </button>
                    ${window.state.notificationCenterOpen ? `
                ...
                ${state.simulatedAppRunning ? `
                  <div class="mb-8 p-6 bg-white rounded-xl border border-slate-200 shadow-sm relative">
                    <div class="flex items-center justify-between pb-4 border-b border-slate-100 mb-6">
                      ...
                      <button onclick="state.simulatedAppRunning=null; window.render();" class="text-xs font-bold text-rose-500 hover:text-rose-700 uppercase tracking-wider cursor-pointer font-sans">Kill Simulation Context</button>
                    </div>
