...
const initializeWorkspaceState = (state) => {
  ...
  window.app.innerHTML = `
    <div class="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-800">
            ...
            <div class="relative group">
              ...
              <div class="absolute left-0 top-full mt-1 w-72 bg-white border border-slate-200 shadow-2xl rounded-xl hidden group-hover:block z-[70] p-4">
                ...
                <div class="space-y-1 max-h-96 overflow-y-auto pr-1">
                  ${portals.map(p => `
                    <button onclick="window.state.activePortalId='${p.id}'; window.render();" class="w-full text-left p-3 rounded-lg hover:bg-slate-50 transition border ${state.activePortalId === p.id ? 'border-blue-100 bg-blue-50/30' : 'border-transparent'}">
                      <p class="text-xs font-bold text-slate-900">${p.name}</p>
                      <p class="text-[10px] text-slate-500 mt-0.5 line-clamp-1">${p.desc}</p>
                    </button>
                  `).join('')}
                </div>
                <div class="mt-4 pt-3 border-t border-slate-100">
                  <button onclick="window.navigate('/gateway')" class="w-full py-2 text-center text-[10px] font-bold text-blue-600 hover:bg-blue-50 rounded uppercase tracking-wider transition">Return to Platform Gateway</button>
                </div>
              </div>
            </div>
      ...
      <!-- Workspace Context Header & Breadcrumbs -->
          ...
          <div class="flex items-center gap-6">
            <nav class="flex items-center gap-1">
              ${navigationEngine.getWorkspaceNavigation(activePortal.id, user).map(nav => `
                <button onclick="window.state.activeModuleId = null; window.switchPortalTab('${nav.id}')" class="px-4 py-1.5 rounded-md text-[11px] font-bold uppercase tracking-widest transition ${currentView === nav.id ? 'bg-slate-100 text-blue-600' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'} cursor-pointer">${nav.label}</button>
              `).join('')}
            </nav>
          </div>
        </div>
                            ...
                            <tr class="hover:bg-slate-50">
                              ...
                              <td class="p-4">
                                <span class="px-2.5 py-0.5 rounded font-bold uppercase text-[9px] tracking-wider
                                  ${sf.status === 'PENDING' || sf.status === 'Pending You' ? 'bg-amber-100 text-amber-800 border border-amber-200' : ''}
                                  ${sf.status === 'APPROVED' || sf.status === 'Approved & Escalated' || sf.status === 'Completed' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : ''}
...
if(typeof window !== 'undefined') window.submitPortalAiQuery = function(_portalName) {
...
if(typeof window !== 'undefined') window.advanceWorkflow = function(wfId, currentStatus) {
  if (window.state) {
    ...
    // Dynamically locate and update the state array entry
    if (window.state.submittedForms) {
      const item = window.state.submittedForms.find(sf => sf.id === wfId);
      if (item) {
